  ...
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all scale-100 flex flex-col max-h-[90vh]">
      ...
      <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
        ...
        <button @click="close" class="text-slate-400 hover:text-slate-600 transition-colors">&times;</button>
      </div>
      ...
      <div class="p-6 space-y-5 overflow-y-auto custom-scrollbar">
        <div class="flex gap-4">
            <div class="flex-1">
                ...
                <input v-model="form.title" type="text" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-100 outline-none font-semibold text-slate-700" placeholder="Ex: Reunião Mensal">
            </div>
            <div>
        ...
        <div class="relative">
            ...
            <div class="relative">
                ...
                <input 
                    ...
                    type="text" 
                    class="w-full pl-9 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-100 outline-none transition-all" 
                    placeholder="Buscar cliente..."
                >
                <button v-if="form.clientName" @click="clearClient" class="absolute right-3 top-2.5 text-xs text-slate-400 hover:text-red-500">Limpar</button>
            </div>
            ...
            <div v-if="showClientList && filteredClients.length > 0" class="absolute z-50 w-full bg-white border border-slate-100 shadow-xl rounded-lg mt-1 max-h-48 overflow-y-auto">
                ...
                >
                    <div class="w-8 h-8 rounded bg-slate-100 text-xs font-bold flex items-center justify-center text-slate-600">{{ client.avatar }}</div>
                    <div>
        ...
        <div class="grid grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
            <div class="col-span-1">
                ...
                <input v-model="form.date" type="date" class="w-full bg-white border border-slate-200 rounded-lg px-2 py-2 text-sm focus:ring-2 focus:ring-blue-100 outline-none">
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-400 uppercase mb-1">Início</label>
                <input v-model="form.time" type="time" class="w-full bg-white border border-slate-200 rounded-lg px-2 py-2 text-sm focus:ring-2 focus:ring-blue-100 outline-none">
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-400 uppercase mb-1">Fim</label>
                <input v-model="form.endTime" type="time" class="w-full bg-white border border-slate-200 rounded-lg px-2 py-2 text-sm focus:ring-2 focus:ring-blue-100 outline-none">
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <select v-model="form.type" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-100 outline-none bg-white">
          ...
          <div>
            ...
            <select v-model="form.agent" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-100 outline-none bg-white">
        ...
        <div>
            <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Notas / Descrição</label>
            <textarea v-model="form.description" rows="3" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-100 outline-none resize-none" placeholder="Detalhes do agendamento..."></textarea>
        </div>
      ...
      <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center shrink-0">
        <button 
            v-if="form.id !== 0" 
            @click="remove" 
            class="text-red-500 hover:text-red-700 hover:bg-red-50 px-3 py-2 rounded-lg text-sm font-bold transition flex items-center gap-1"
        >
            🗑️ Excluir
        </button>
        <div v-else></div>
        ...
        <div class="flex gap-3">
            <button @click="close" class="px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-200 rounded-lg transition">Cancelar</button>
            <button @click="save" class="px-6 py-2 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-lg shadow-blue-200 transition transform active:scale-95">
