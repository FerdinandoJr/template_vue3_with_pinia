  ...
  <div class="flex-1 flex flex-col h-full bg-[#efeae2] relative border-r border-slate-200">
    <div class="px-6 py-3 border-b border-slate-200 flex justify-between items-center bg-white z-10 shadow-sm shrink-0">
      ...
      >
        ...
        <div>
          <h3 class="font-bold text-slate-800 text-[14px] leading-tight mb-0.5 group-hover:text-blue-600 transition-colors">{{ contact.name }}</h3>
      ...
      <div class="flex gap-2">
        <template v-if="contact.status !== 'online'">
          <button 
        ...
        <template v-else>
          <button @click="$emit('adicionar')" class="px-4 py-2 bg-blue-50 border border-blue-200 text-blue-700 rounded-lg text-[12px] font-bold hover:bg-blue-100 transition-colors flex items-center gap-1.5">
    ...
    <div class="shrink-0 flex flex-col bg-[#f0f2f5] relative">
      <div v-if="attachedFile" class="absolute bottom-[100%] left-0 w-full p-3 bg-slate-50 border-t border-slate-200 flex items-center gap-3 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs text-slate-500">{{ (attachedFile.size / 1024).toFixed(1) }} KB</p>
        </div>
        <button @click="attachedFile = null" class="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors">
      ...
      <template v-if="contact.status !== 'online'">
        <div class="w-full text-center py-5 text-slate-500 text-sm font-medium border-t border-slate-200">
          ...
          Assuma o chamado na fila para enviar mensagens.
        </div>
      </template>
      ...
      <template v-else>
        ...
        <div :class="['px-4 py-3 shrink-0 flex items-center gap-2 transition-colors relative z-0', isInternalNote ? 'bg-amber-100 border-t border-amber-300' : 'bg-white border-t border-slate-200']">
          <label :class="['w-10 h-10 flex items-center justify-center transition-colors cursor-pointer rounded-full hover:bg-slate-100', isInternalNote ? 'text-amber-600' : 'text-slate-500']">
          ...
          <input 
            ...
            @keyup.enter="send" 
            @paste="handlePaste"
            :placeholder="isInternalNote ? 'Digite uma nota...' : 'Mensagem...'" 
...
const handleFileSelect = (event: Event) => {
  ...
  if (target.files && target.files.length > 0) {
    attachedFile.value = target.files[0] || null;
    target.value = ''; 
  }
};
...
const handlePaste = (event: ClipboardEvent) => {
  if (event.clipboardData && event.clipboardData.files.length > 0) {
    attachedFile.value = event.clipboardData.files[0] || null;
  }
};
